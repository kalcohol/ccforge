...
| `asan` | `forge-asan` | `Containerfile.asan` | AddressSanitizer + UBSan gate for the execution and `forge::` utility subsets. |
...
- Sanitizer images intentionally run only the execution and `forge::` utility
  subsets; they are lifetime/race gates, not full feature-matrix gates.

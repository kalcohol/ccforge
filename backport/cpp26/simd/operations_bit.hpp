...
template<class T, class Abi>
constexpr rebind_t<int, basic_vec<T, Abi>> popcount(const basic_vec<T, Abi>& value) noexcept
    requires(is_integral<T>::value && detail::has_popcount<T>::value) {
    using result_type = rebind_t<int, basic_vec<T, Abi>>;
    result_type result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {
        detail::set_lane(result, i, static_cast<int>(std::popcount(detail::to_unsigned_bits(value[i]))));
    }
    return result;
}

template<class T, class Abi>
constexpr rebind_t<int, basic_vec<T, Abi>> countl_zero(const basic_vec<T, Abi>& value) noexcept
    requires(is_integral<T>::value && detail::has_countl_zero<T>::value) {
    using result_type = rebind_t<int, basic_vec<T, Abi>>;
    result_type result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {
        detail::set_lane(result, i, static_cast<int>(std::countl_zero(detail::to_unsigned_bits(value[i]))));
    }
    return result;
}

template<class T, class Abi>
constexpr rebind_t<int, basic_vec<T, Abi>> countl_one(const basic_vec<T, Abi>& value) noexcept
    requires(is_integral<T>::value && detail::has_countl_one<T>::value) {
    using result_type = rebind_t<int, basic_vec<T, Abi>>;
    result_type result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {
        detail::set_lane(result, i, static_cast<int>(std::countl_one(detail::to_unsigned_bits(value[i]))));
    }
    return result;
}

template<class T, class Abi>
constexpr rebind_t<int, basic_vec<T, Abi>> countr_zero(const basic_vec<T, Abi>& value) noexcept
    requires(is_integral<T>::value && detail::has_countr_zero<T>::value) {
    using result_type = rebind_t<int, basic_vec<T, Abi>>;
    result_type result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {
        detail::set_lane(result, i, static_cast<int>(std::countr_zero(detail::to_unsigned_bits(value[i]))));
    }
    return result;
}

template<class T, class Abi>
constexpr rebind_t<int, basic_vec<T, Abi>> countr_one(const basic_vec<T, Abi>& value) noexcept
    requires(is_integral<T>::value && detail::has_countr_one<T>::value) {
    using result_type = rebind_t<int, basic_vec<T, Abi>>;
    result_type result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {
        detail::set_lane(result, i, static_cast<int>(std::countr_one(detail::to_unsigned_bits(value[i]))));
    }
    return result;
}

template<class T, class Abi>
constexpr rebind_t<int, basic_vec<T, Abi>> bit_width(const basic_vec<T, Abi>& value) noexcept
    requires(is_integral<T>::value && detail::has_bit_width<T>::value) {
    using result_type = rebind_t<int, basic_vec<T, Abi>>;
    result_type result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {
        detail::set_lane(result, i, static_cast<int>(std::bit_width(detail::to_unsigned_bits(value[i]))));
    }
    return result;
}
...
constexpr typename basic_vec<T, Abi>::mask_type has_single_bit(const basic_vec<T, Abi>& value) noexcept
    requires(is_integral<T>::value && detail::has_has_single_bit<T>::value) {
...
constexpr basic_vec<T, Abi> bit_floor(const basic_vec<T, Abi>& value) noexcept
    requires(is_integral<T>::value && detail::has_bit_floor<T>::value) {
    basic_vec<T, Abi> result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {
...
constexpr basic_vec<T, Abi> bit_ceil(const basic_vec<T, Abi>& value) noexcept
    requires(is_integral<T>::value && detail::has_bit_ceil<T>::value) {
    basic_vec<T, Abi> result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {
...
template<class T, class Abi>
constexpr basic_vec<T, Abi> rotl(const basic_vec<T, Abi>& value, simd_size_type shift) noexcept
    requires(is_integral<T>::value && detail::has_rotl<T>::value) {
    basic_vec<T, Abi> result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {
        detail::set_lane(result, i, static_cast<T>(std::rotl(detail::to_unsigned_bits(value[i]), static_cast<int>(shift))));
    }
    return result;
}

template<class T, class Abi>
constexpr basic_vec<T, Abi> rotl(const basic_vec<T, Abi>& value, const basic_vec<T, Abi>& shift) noexcept
    requires(is_integral<T>::value && detail::has_rotl<T>::value) {
    basic_vec<T, Abi> result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {
...
template<class T, class Abi>
constexpr basic_vec<T, Abi> rotr(const basic_vec<T, Abi>& value, simd_size_type shift) noexcept
    requires(is_integral<T>::value && detail::has_rotr<T>::value) {
    basic_vec<T, Abi> result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {
        detail::set_lane(result, i, static_cast<T>(std::rotr(detail::to_unsigned_bits(value[i]), static_cast<int>(shift))));
    }
    return result;
}

template<class T, class Abi>
constexpr basic_vec<T, Abi> rotr(const basic_vec<T, Abi>& value, const basic_vec<T, Abi>& shift) noexcept
    requires(is_integral<T>::value && detail::has_rotr<T>::value) {
    basic_vec<T, Abi> result;
    for (simd_size_type i = 0; i < basic_vec<T, Abi>::size; ++i) {

...
if(_FORGE_PACKAGE_MODE)
    ...
    if(TARGET forge::std)
        get_target_property(
            _forge_source_wrapper_owner forge::std _CCFORGE_SOURCE_WRAPPER)
        if(NOT _forge_source_wrapper_owner STREQUAL "std")
            message(FATAL_ERROR
                "CC Forge cannot create forge::std because that target already exists")
        endif()
    else()
        add_library(forge::std INTERFACE IMPORTED GLOBAL)
        set_property(TARGET forge::std PROPERTY _CCFORGE_SOURCE_WRAPPER std)
        set_property(TARGET forge::std PROPERTY
            INTERFACE_LINK_LIBRARIES forge_std)
    endif()
    unset(_forge_source_wrapper_owner)
endif()
...
if(_FORGE_PACKAGE_MODE)
    ...
    if(TARGET forge::forge)
        get_target_property(
            _forge_source_wrapper_owner forge::forge _CCFORGE_SOURCE_WRAPPER)
        if(NOT _forge_source_wrapper_owner STREQUAL "forge")
            message(FATAL_ERROR
                "CC Forge cannot create forge::forge because that target already exists")
        endif()
    else()
        add_library(forge::forge INTERFACE IMPORTED GLOBAL)
        set_property(TARGET forge::forge PROPERTY _CCFORGE_SOURCE_WRAPPER forge)
        set_property(TARGET forge::forge PROPERTY
            INTERFACE_LINK_LIBRARIES forge)
    endif()
    unset(_forge_source_wrapper_owner)
endif()

...
| Forge stop-token erasure | `forge::any_stop_token` | Type-erased per-operation token | Forge runtime wrappers 擦除 downstream receiver stop token 并保持 callback lifetime 正确。 | 内部 control block allocator-neutral；不向 `namespace std` 注入非标准名字。 |
| Execution consumers | `sync_wait` | No cancellation support | receiver env 暴露 start/delegation scheduler；缺失 stop query 因而得到 `never_stop_token`；`set_stopped` 返回 empty optional。 | Blocking consumer，不是 cancellation framework，也不向 child 提供 caller-owned stop source。 |
...
| Execution scope | `counting_scope::token::wrap` | Fused token / composed stop source | child env 暴露 fused stop token；scope stop 与 downstream receiver/env stop 任一请求都会让 child 观察 stop。 | 融合状态、stop callback 与 child operation 在 `connect()` 期间建立；构造或 child connect 失败同步抛出，不伪装成 stopped completion。 |
...
| Forge scheduler | `single_thread_context` / `resource_context` | Primitive-owned shutdown | 组合内部 runtime/scope，destructor shutdown + wait；resource-context stop 同时传播到 scope 与 pending runtime timers。 | Direct pool work 没有 runtime-owned stop token，仍 drain；owner 负责不在 own worker/completion 内销毁。 |
...
| Forge erasure | `erased_sender` | Stop-token forwarding | erased receiver env 把 downstream stop token 暴露为 `forge::any_stop_token`。 | 只承诺 bounded env model 中的 stop token，不擦除任意 env query。 |
